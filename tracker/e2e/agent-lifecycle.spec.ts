...
async function agentTools(request: APIRequestContext) {
  const response = await request.get("/api/tracker/v1/settings/agents/tools", { headers: headers() });
  expect(response.ok()).toBeTruthy();
  return (await response.json()).data.tools as Array<Record<string, any>>;
}
...
test.describe("isolated agent lifecycle", () => {
  ...
  test("installs all providers, recovers fallback, and manages isolated accounts", async ({
    ...
    await expect(page.getByText("Managed preferred; using System PATH")).toBeVisible();
    await expect(page.getByRole("checkbox", { name: "Automatic account failover" })).not.toBeChecked();
    ...
    for (const agent of agents.filter((agent) => agent !== "codex")) {
      const response = await request.post(`/api/tracker/v1/settings/agents/${agent}/install`, {
        headers: headers(),
      });
    ...
    for (const agent of agents) {
      ...
      expect(tool.source.preferred, `${agent} preferred source`).toBe("managed");
      expect(tool.status.version).toContain("1.0.0");
      expect(path.resolve(tool.status.path).startsWith(path.resolve(dataRoot!))).toBeTruthy();
      expect((await stat(tool.status.path)).mode & 0o111, `${agent} executable mode`).not.toBe(0);
    }
    ...
    for (const account of [
      ...
      const response = await request.post("/api/tracker/v1/settings/agents/codex/accounts", {
        headers: headers(),
        data: { ...account, authentication_status: "authenticated", access_token: SECRET_SENTINEL },
      });
    ...
    await writeFile(credentialPath, JSON.stringify({ token: SECRET_SENTINEL }), { mode: 0o600 });
    ...
    const failover = page.getByRole("checkbox", { name: "Automatic account failover" });
    await failover.check();
    await expect(failover).toBeChecked();
    const loadedSettings = await request.get("/api/tracker/v1/settings", { headers: headers() });
    expect((await loadedSettings.json()).data.agent_cli.codex.failover_enabled).toBe(true);
    ...
    await fixtureControl(request, { version: "2.0.0", mode: "checksum_mismatch" });
    const failedUpdate = await request.post("/api/tracker/v1/settings/agents/codex/update", {
      headers: headers(),
    });
    expect(failedUpdate.status()).toBe(422);

    tools = await agentTools(request);
    expect(tools.find((entry) => entry.id === "codex")!.status.version).toContain("1.0.0");

    await fixtureControl(request, { version: "2.0.0", mode: "ok" });
    const update = await request.post("/api/tracker/v1/settings/agents/codex/update", {
      headers: headers(),
    });
    expect(update.ok()).toBeTruthy();

    tools = await agentTools(request);
    expect(tools.find((entry) => entry.id === "codex")!.status.version).toContain("2.0.0");

    const accounts = await request.get("/api/tracker/v1/settings/agents/codex/accounts", {
      headers: headers(),
    });
