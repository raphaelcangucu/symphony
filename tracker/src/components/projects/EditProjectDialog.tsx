    ...
    <div className="space-y-3 rounded-lg border p-3">
      ...
      {projectId ? (
        <p className="rounded-md bg-muted/30 px-3 py-2 text-xs">
          Connected board: <code>{projectId}</code>
          {typeof projectNumber === "number" ? <span className="text-muted-foreground"> · #{projectNumber}</span> : null}
        </p>
      ) : (
        <p className="text-xs text-amber-600 dark:text-amber-400">No board selected yet.</p>
      )}
      ...
      <GitHubProjectPicker
        onSelect={(board) =>
          onConfigChange({
            ...
            repo: board.repoNameWithOwner ?? configString(config, "repo"),
          })
        }
      />
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <div className="space-y-1">
          ...
          <Input
            id="edit-github-repo"
            value={configString(config, "repo")}
