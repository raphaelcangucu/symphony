...
vi.mock("@/components/sessions/AssistantSessionTabContent", () => ({
  AssistantSessionTabContent: (props: { threadId: number }) => (
    <div
      aria-label="mock execution session panel"
...
vi.mock("@/components/issues/issue-detail/IssueAuthoringSessionPanel", () => ({
  IssueAuthoringSessionPanel: (props: { issue: { identifier: string } }) => (
    <div
      aria-label="mock authoring session panel"
      data-issue={props.issue.identifier}
    />
  ),
}));
vi.mock("@/components/assistant/IssueAuthoringPanel", () => ({
  IssueAuthoringPanel: (props: {
    ...
    <div
      aria-label="mock new issue authoring panel"
      data-identifier={props.identifier ?? ""}
    >
      <button
        type="button"
        onClick={() => props.onIssueCreated?.({ identifier: "DEMO-2" })}
      >
...
function WorkspaceQueryHarness({ projectSlug }: { projectSlug: string }) {
  ...
  const activeAuthoringIdentifier =
    exec && section === "authoring" ? exec : null;
  const activeExecutionIdentifier =
    exec && section === "execution" ? exec : null;
...
describe("ProjectSessionsPanel", () => {
  ...
  it("renders workspace cards and related chats", () => {
    ...
    expect(
      screen.getAllByText("Saved launcher work").length,
    ).toBeLessThanOrEqual(2);
    expect(
      screen.getAllByRole("button", { name: /DEMO-1 Saved launcher work/i }),
    ).toHaveLength(1);
    expect(screen.getAllByText("Issue authoring chat").length).toBeGreaterThan(
      0,
    );
    ...
    const demoCard = screen.getByRole("button", {
      name: /DEMO-1 Saved launcher work/i,
    });
    expect(demoCard).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /DEMO-2 Issue authoring chat/i }),
    ).toBeInTheDocument();
    const listItem = demoCard.closest("li");
    expect(listItem).not.toBeNull();
    fireEvent.click(
      within(listItem as HTMLElement).getByRole("button", {
        name: /^Expand$/i,
      }),
    );
    expect(
      screen.getAllByRole("link", { name: "Open issue DEMO-1" })[0],
    ).toHaveAttribute("href", "/projects/demo/board/issues/DEMO-1/sessions");
  ...
  it("archives related assistant sessions from the project sessions list", async () => {
    ...
    fireEvent.click(
      await screen.findByRole("button", { name: /Archive session/i }),
    );
  ...
  it("lists the orchestrator execution as a single thread row (no synthetic Autonomous row)", async () => {
    ...
    expect(
      within(detail).queryByRole("button", { name: /Open autonomous run/i }),
  ...
  it("opens the execution thread row via the real orchestrator session id", async () => {
    ...
    fireEvent.click(
      await within(listItem as HTMLElement).findByRole("button", {
        name: "Saved launcher work",
      }),
    );

    expect(
      await screen.findByLabelText("mock execution session panel"),
    ).toHaveAttribute("data-thread", "42");
  ...
  it("archives the execution thread from its row like any other thread", async () => {
    ...
    await waitFor(() =>
      expect(archiveAssistantThread).toHaveBeenCalledWith(42),
    );
  });
  ...
  it("resolves the legacy exec deep link to the orchestrator session workspace", async () => {
    renderWithI18n(
      <MemoryRouter
        initialEntries={[
          "/projects/demo/workspaces?exec=DEMO-1&surface=autonomous",
        ]}
      >
        <ProjectSessionsPanel
          projectSlug="demo"
          activeExecutionIdentifier="DEMO-1"
        />
      </MemoryRouter>,
    );
  ...
  it("clears the legacy exec deep link when no orchestrator session exists", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByLabelText("mock execution session panel"),
      ).not.toBeInTheDocument();
    });
  });
  ...
  it("opens the interactive authoring thread as a real session", async () => {
    ...
    const demoCard = screen.getByRole("button", {
      name: /DEMO-2 Issue authoring chat/i,
    });
    const listItem = demoCard.closest("li");
    expect(listItem).not.toBeNull();
    fireEvent.click(
    ...
    fireEvent.click(
      await within(listItem as HTMLElement).findByRole("button", {
        name: "Issue authoring chat",
      }),
    );

    expect(
      await screen.findByLabelText("mock execution session panel"),
    ).toHaveAttribute("data-thread", "2");
  });
  ...
  it("restores the authoring session from the exec identifier in the URL", () => {
    renderWithI18n(
      <MemoryRouter
        initialEntries={[
          "/projects/demo/workspaces?exec=DEMO-1&surface=session",
        ]}
      >
        <ProjectSessionsPanel
          projectSlug="demo"
          activeAuthoringIdentifier="DEMO-1"
        />
      </MemoryRouter>,
    );
    ...
    expect(screen.getByRole("tab", { name: /DEMO-1/i })).toBeInTheDocument();
    expect(
      screen.getByLabelText("mock authoring session panel"),
    ).toHaveAttribute("data-issue", "DEMO-1");
  });
  ...
  it("opens the ephemeral new issue tab from the new=1 workspace query", () => {
    ...
    expect(
      screen.getByRole("tab", { name: /New issue with assistant/i }),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("mock new issue authoring panel"),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("mock new issue authoring panel"),
    ).toHaveAttribute("data-identifier", "");
  });
  ...
  it("morphs the new issue tab into an authoring session when an issue is created", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Simulate issue created" }),
    );

    expect(
      await screen.findByLabelText("mock authoring session panel"),
    ).toHaveAttribute("data-issue", "DEMO-2");
    expect(
      screen.queryByLabelText("mock new issue authoring panel"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("tab", { name: /New issue with assistant/i }),
    ).not.toBeInTheDocument();
  ...
  it("prefers an exec authoring session over the new-issue query", () => {
    renderWithI18n(
      <MemoryRouter
        initialEntries={[
          "/projects/demo/workspaces?new=1&exec=DEMO-1&agent=authoring",
        ]}
      >
        <WorkspaceQueryHarness projectSlug="demo" />
      </MemoryRouter>,
  ...
  it("opens the orchestrator session when executionSessionId arrives after reload", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Refresh hook snapshot" }),
    );
    ...
    expect(
      screen.queryByRole("tab", {
        name: /Saved launcher work · (Execution|Autonomous|Session)/i,
      }),
    ).not.toBeInTheDocument();
  });
  ...
  it("resumes a saved session from the workspace row menu", async () => {
    ...
    await waitFor(() =>
      expect(dispatchIssueAgent).toHaveBeenCalledWith("demo", "DEMO-1", {
        action: "resume",
      }),
    );
    expect(refetch).toHaveBeenCalled();
