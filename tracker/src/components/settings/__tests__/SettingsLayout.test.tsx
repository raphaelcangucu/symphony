    ...
    <MemoryRouter initialEntries={[initialPath]}>
      <Routes>
        <Route path="/settings" element={<SettingsLayout />}>
          ...
          <Route
            path="experimental"
            element={<div>Experimental content</div>}
          />
          <Route path="templates" element={<div>Templates content</div>} />
          <Route
            path="templates/:slug"
            element={<div>Template edit content</div>}
          />
...
describe("SettingsLayout", () => {
  it("renders grouped navigation headers", async () => {
    ...
    expect(
      screen.getByRole("combobox", { name: "Settings sections" }),
  ...
  it("uses a compact grouped section picker for mobile navigation", async () => {
    renderSettingsLayout("/settings/agents/codex");

    const picker = await screen.findByRole("combobox", {
      name: "Settings sections",
    });
    expect(picker).toHaveValue("/settings/agents/codex");

    fireEvent.change(picker, { target: { value: "/settings/appearance" } });

    expect(await screen.findByText("Appearance content")).toBeTruthy();
  ...
  it("highlights the general section on /settings", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "Templates" }).className,
  ...
  it("highlights templates when editing a template", async () => {
    ...
    expect(screen.getByRole("link", { name: "Templates" }).className).toContain(
      "bg-accent",
    );
    expect(
      screen.getByRole("link", { name: "General" }).className,
    ).not.toContain("bg-accent");
  });
  ...
  it("highlights a supported agent page", async () => {
    ...
    expect(screen.getByRole("link", { name: "Codex" }).className).toContain(
      "bg-accent",
    );
  });
  ...
  it("highlights providers on /settings/providers", async () => {
    ...
    expect(screen.getByRole("link", { name: "Providers" }).className).toContain(
      "bg-accent",
    );
  });
  ...
  it("highlights usage on /settings/usage", async () => {
    ...
    expect(screen.getByRole("link", { name: "Usage" }).className).toContain(
      "bg-accent",
    );
  });
  ...
  it("highlights experimental on /settings/experimental", async () => {
    ...
    expect(
      screen.getByRole("link", { name: "Experimental" }).className,
    ).toContain("bg-accent");
  });
  ...
  it("highlights gateways on /settings/gateways", async () => {
    ...
    expect(screen.getByRole("link", { name: "Gateways" }).className).toContain(
      "bg-accent",
    );
  });
});
