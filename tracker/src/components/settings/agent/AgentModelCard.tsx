...
export function AgentModelCard({ agent, label, model }: AgentModelCardProps) {
  ...
  const [selected, setSelected] = useState<string>(model.selected ?? CLI_DEFAULT_VALUE);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("settings.agentTool.settings", { name: label })}</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-0.5">
            <p className="text-sm font-medium">{t("settings.agentTool.model.title")}</p>
          ...
          >
            <option value={CLI_DEFAULT_VALUE}>{t("settings.agentTool.model.cliDefault")}</option>
