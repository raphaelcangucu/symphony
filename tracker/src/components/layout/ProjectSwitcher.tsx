...
export function ProjectSwitcher({ projectSlug, title }: ProjectSwitcherProps) {
  ...
  return (
    <DropdownMenu open={open} onOpenChange={handleOpenChange}>
      <DropdownMenuTrigger
        className="group flex max-w-[9rem] items-center gap-1.5 rounded-md px-2 py-1 text-left outline-none transition-colors hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 sm:max-w-[16rem]"
      ...
      <DropdownMenuContent align="start" className="w-64">
        ...
        {loading && entries.length === 0 ? (
          <div className="px-2 py-1.5 text-sm text-muted-foreground">
            {t("layout.projectHeader.loadingProjects")}
          </div>
        ) : null}
