...
describe("IssueSessionsTab", () => {
  ...
  it("navigates to the project sessions page when opening a session", async () => {
    ...
    expect(navigateMock).toHaveBeenCalledWith("/projects/macro-markets/sessions?exec=MAC-510&agent=execution");

    await user.click(screen.getByText("Build pass 1"));
    expect(navigateMock).toHaveBeenCalledWith("/projects/macro-markets/sessions/12");

    await user.click(screen.getByText("Issue plan draft"));
    expect(navigateMock).toHaveBeenCalledWith("/projects/macro-markets/sessions?exec=MAC-510&agent=authoring");
  });
});
