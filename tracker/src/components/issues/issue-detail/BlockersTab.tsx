...
export function BlockersTab({ projectSlug, issue }: BlockersTabProps) {
  ...
  async function addBlocker() {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : "Failed to add blocker");
    }
  }
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg border p-3">
        <div className="mb-2 text-sm font-medium">Add blocker</div>
        <div className="flex gap-2">
          <Input
            aria-label="Blocking issue identifier"
      ...
      {issue.blockedBy.length > 0 ? (
        <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
          <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-amber-700">Blocked by</div>
          <div className="flex flex-wrap gap-2">
            {issue.blockedBy.map((blocker) => <Badge key={blocker.id} variant="secondary">{blocker.identifier}</Badge>)}
          </div>
        </div>
      ) : null}
      {blockers.length === 0 ? <p className="text-sm text-muted-foreground">No blocker records.</p> : null}
      {blockers.map((blocker) => (
        <article key={blocker.id} className="rounded-lg border p-3 text-sm">
          <div className="mb-1 flex items-center justify-between gap-2">
            <span className="font-medium">{blocker.blockingIssueIdentifier || "External blocker"}</span>
