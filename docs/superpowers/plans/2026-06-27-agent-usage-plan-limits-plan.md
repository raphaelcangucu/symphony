...
>
> **AS-BUILT ADDENDUM (Claude/Cursor reality):** The original "one passive path covers all agents" was wrong — only **Codex** emits usable rate-limit data on the stream. **Claude's** `rate_limit` event is dropped by its adapter (`claude/coding_agent.ex:232-234` reads only a top-level `:rate_limits`, but the data is inside `params`), and its real shape is the **Anthropic OAuth usage API** (`five_hour`/`seven_day`/`seven_day_sonnet`), not the Codex `primary`/`secondary`. Verified against the Jean source (`/tmp/jean-analysis`): Jean fetches Claude usage via `GET https://api.anthropic.com/api/oauth/usage` (bearer from `~/.claude/.credentials.json`, header `anthropic-beta: oauth-2025-04-20`) and Codex via `chatgpt.com/backend-api/wham/usage`; **Jean has no Cursor usage**. So we added an **active probe** `SymphonyElixir.Claude.Usage` (read creds → call the OAuth usage API → normalize `five_hour → :session`, `seven_day → :weekly`, `seven_day_sonnet → :sonnet_weekly`, `extra_usage → credits`), triggered best-effort from the usage endpoint (60s backoff; off under `:test`; fails soft; token refresh = follow-up). **Cursor** stays "unavailable" (matches Jean). E2E proven for Codex via `agent_usage_e2e_test.exs` (POST report → Registry capture → store → endpoint).
