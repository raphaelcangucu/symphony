...
| D8 | **WebSocket/HMR supported**: HTTP requests proxy via `reverse_proxy_plug`; WebSocket upgrade requests proxy via the separate `reverse_proxy_plug_websocket` package (the HTTP package does not handle WS). The plug detects the `upgrade: websocket` header and dispatches accordingly. | Vite/Next dev servers require WS for HMR. |
