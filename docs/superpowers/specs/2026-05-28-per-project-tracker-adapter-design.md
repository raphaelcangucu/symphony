...
| D10 | **Identifier strategy on remotes** = use the remote system's identifier verbatim (`#123` for GitHub, `LIN-42` for Linear). The Symphony local-only `next_identifier/1` is not used for remote projects. | Keeps deep-linking to remote tools simple. |
