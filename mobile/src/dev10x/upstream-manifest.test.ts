...
const importedRoot = resolve(__dirname);

describe("pinned Orca presentation import", () => {
  it("records the exact upstream revision", () => {
    expect(readFileSync(resolve(mobileRoot, "UPSTREAM_PROVENANCE.md"), "utf8")).toContain(
  ...
  it.each([
    ...
    expect(existsSync(resolve(importedRoot, relativePath))).toBe(true);
  });
});
