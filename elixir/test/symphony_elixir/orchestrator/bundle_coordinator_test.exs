defmodule SymphonyElixir.Orchestrator.BundleCoordinatorTest do
  ...
  defp same_repo_child_bundle do
    %ExecutionBundle{
      ...
      parent: "macro/app#1",
      units: [
        %{id: "be", type: :child_run, issue: "macro/app#2", repo: "macro/app", produces: ["api"], consumes: [], depends_on: [], deliverable: "pr"},
        %{id: "fe", type: :child_run, issue: "macro/app#3", repo: "macro/app", produces: [], consumes: ["api"], depends_on: ["be"], deliverable: "pr"}
      ],
      shared_contracts: [%{id: "api", owner_unit: "be", consumers: ["fe"], kind: "graphql", artifact: nil, status: :draft}]
    }
  end
