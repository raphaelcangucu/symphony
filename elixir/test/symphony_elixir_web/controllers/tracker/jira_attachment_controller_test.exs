defmodule SymphonyElixirWeb.Tracker.JiraAttachmentControllerTest do
  ...
  setup do
    ...
    previous_delete_request_fun = Application.get_env(:symphony_elixir, :jira_attachment_delete_request_fun)
    ...
    on_exit(fn ->
      ...
      restore_app_env(:jira_attachment_delete_request_fun, previous_delete_request_fun)
  ...
  test "deletes an attachment and returns 204" do
    Application.put_env(:symphony_elixir, :jira_attachment_delete_request_fun, fn _url, _headers ->
      {:ok, %{status: 204}}
    end)
    ...
    conn =
      build_conn()
      |> put_req_header("authorization", "Bearer secret")
      |> delete("/api/tracker/v1/projects/advising/jira/attachments/10501")

    assert conn.status == 204
    assert conn.resp_body == ""
  end

  test "maps a remote delete 404 to attachment_not_found" do
    Application.put_env(:symphony_elixir, :jira_attachment_delete_request_fun, fn _url, _headers ->
      {:ok, %{status: 404}}
    end)
    ...
    conn =
      build_conn()
      |> put_req_header("authorization", "Bearer secret")
      |> delete("/api/tracker/v1/projects/advising/jira/attachments/does-not-exist")
