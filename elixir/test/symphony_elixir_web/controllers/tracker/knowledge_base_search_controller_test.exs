defmodule SymphonyElixirWeb.Tracker.KnowledgeBaseSearchControllerTest do
  ...
  test "GET project search returns ranked matches" do
    put(authorized_conn(), "/api/tracker/v1/projects/acme/kb/repos/web/pages/note.md", %{
      "frontmatter" => %{"title" => "Note"},
      "body" => "a unique giraffe term"
    })
  ...
  test "GET project search with repo filter" do
    put(authorized_conn(), "/api/tracker/v1/projects/acme/kb/repos/web/pages/x.md", %{
      "frontmatter" => %{},
      "body" => "filtered llama"
    })
  ...
  defp configure_isolated_workspace_root do
    root = Path.join(System.tmp_dir!(), "kb-sctrl-#{System.unique_integer([:positive])}")
    File.mkdir_p!(root)
