...
test("keeps 18 historical runs plus current-default and Dev10x high matrices", () => {
  assert.equal(RUN_MATRIX.length, 30);
  assert.equal(new Set(RUN_MATRIX.map((run) => run.id)).size, 30);
  assert.deepEqual(
  ...
  assert.equal(
    RUN_MATRIX.filter((run) => run.matrix === "dev10x-brand-high").length,
    6,
  );
  assert.equal(
